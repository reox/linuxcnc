...
static int hm2_eth_receive_queued_reads(hm2_lowlevel_io_t *this) {
    ...
    if(recv != board->queue_buff_size) {
        ...
        if(board->read_error_counter < 10)
            board->read_error_counter ++;
        return board->read_error_counter < 10;
        return 0;
    }

    if(board->read_error_counter < 2)
        board->read_error_counter = 0;
    else
        board->read_error_counter -= 2;
